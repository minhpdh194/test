...
use App\Http\Controllers\Clickers\Shared;

trait Booster
{
    public static function buyBoosterPack(Request $request)
    {
        $request->validate([
        ...
        try {
            DB::transaction(function () use ($user, $boosterPack) {
                ...
                if ($boosterActiveUntil > $currentTime) {
                    if (Booster::isValidUpgrade($user, $boosterPack)) {
                        Booster::deactivateCurrentBooster($user);
                    } else {
                        throw new \InvalidArgumentException("Cannot downgrade or repurchase the same booster pack while one is active.");
                    }
                }

                Booster::activateBoosterPack($user, $boosterPack);
                $user->save();
            });
    ...
    public static function buyBooster(Request $request)
    {
        $request->validate([
        ...
        $cost = Shared::getBoosterCost($user, $boosterType);
        ...
        try {
            ...
            return response()->json([
                ...
                'next_multi_tap_cost' => Shared::getBoosterCost($user, 'multi_tap'),
                'next_energy_limit_cost' => Shared::getBoosterCost($user, 'energy_limit'),
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
    ...
    public static function useDailyBooster(Request $request)
    {
        $user = $request->user();
    ...
    private static function getCurrentBoosterPack($user)
    {
    ...
    private static function deactivateCurrentBooster($user)
    {
    ...
    private static function activateBoosterPack($user, $boosterPack)
    {
        switch($boosterPack) {
    ...
    private static function isValidUpgrade($user, $newPack)
    {
        $packValues = [
            'booster_pack_2x' => 2,
            'booster_pack_3x' => 3,
        ...
        $currentPack = Booster::getCurrentBoosterPack($user);
        if($currentPack) {
