...
const TradingItem = ({ spots, perfs, onValidatePosition }: TradingItemProps) => {
    ...
    const handleSelectedBonusesChange = async (bonuses: Bonus[]): Promise<void> => {
        if (bonuses.length > 0) {
            ...
            if (!selectedOptions[bonusPositionId] || !leverages[bonusPositionId] || leverages[bonusPositionId] == 0) {
                ...
                // Position will be created with the selected bonuses
    ...
    const handleValidate = async (pairId: number): Promise<void> => {
        try {
            ...
            if (!selectedOptions[pairId] || !leverages[pairId]) return;
    ...
    return (
        <div className="mt-3">
            {globalThis.userProfile.unlocked_pairs.map((pair) => (
                <div key={pair.id} className="bg-[#32363C] bg-opacity-90 rounded-xl mb-3">
