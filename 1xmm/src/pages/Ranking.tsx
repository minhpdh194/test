    ...
    async function getUserRanking() {
        ...
        try {
            ...
            const usersResponse = await $http.get('/get-left-users', {
                params: {
                    played_users: response.data.map((user: any) => user.telegram_user_id),
                }
            });
            ...
            usersResponse.data.forEach((userData: any) => {
                const newData = {
                    ...
                    amount_of_tokens: 0,
                    avatar_id: userData.user_profiles.avatar_id,
                };
        ...
        >
            ...
            <div className="mt-4 mb-8">
                <div className="flex gap-2 w-100 text-sm">
                    <div
                        className={`bg-[#32363C] flex-1 text-center py-2 rounded-lg ${selectedFilter === 0 ? 'border border-white' : ''
                            }`}
                        onClick={() => setSelectedFilter(0)}
                    >
                        <span className="text-[#F79841] fw-semibold">{t(`${ranking}.total`)}</span>
                        <span className="fw-light block">{Utils.formatDate(selectedDate.toUTCString())}</span>
                    </div>

                    <div
                        className={`bg-[#32363C] flex-1 text-center py-2 rounded-lg ${selectedFilter === 1 ? 'border border-white' : ''
                            }`}
                    ...
                    <div
                        className={`bg-[#32363C] flex-1 text-center py-2 rounded-lg ${selectedFilter === 2 ? 'border border-white' : ''
                            }`}
                        ...
                        <div className="w-100 text-sm p-3" key={index}>
                            <div className="row p-2 rounded-xl items-center bg-[#19203b]">
                                ...
                                <div className="col-5">
                                    ...
                                    <span className="fw-light block text-xs">{formatNumber(user)}</span>
                                </div>
                            </div>
                        </div>
                    )
